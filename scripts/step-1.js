...
const useAtomic = (await rl.question("Using atomic desing? (y/n):")) === "y";
...
for (const contentType of contentTypes) {
  ...
  const atomicType = getAtomicPrefix(contentType.name);
  const schema = transformFromContentfulToStrapi(contentType, type, atomicType);
...
function transformFromContentfulToStrapi(contentType, type, atomicType) {
  const schema = {
  ...
  if (type !== "C") {
    ...
    schema.collectionName = `components_${atomicType}_${schema.collectionName}`;
  }

  contentType.fields.forEach((field) => {
    schema.attributes[field.id] =
